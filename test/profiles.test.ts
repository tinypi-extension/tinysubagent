...
import { CURRENT_PROFILE, JSONC_CONFIG_FILENAME, type TinysubagentConfig } from "../src/config.ts";
import {
...
function enabled(profiles: TinysubagentConfig["profiles"]): TinysubagentConfig {
	return { enableProfiles: true, profiles };
}
...
test("an omitted profile inherits the parent, whether or not profiles are enabled", () => {
	...
	for (const name of [undefined, "", "   "]) {
		...
		const disabled = resolveProfile({ enableProfiles: false, profiles: {} }, name, PARENT);
...
test("when profiles are disabled an omitted profile inherits the parent", () => {
	const disabled: TinysubagentConfig = { enableProfiles: false, profiles: {} };
...
test("when profiles are disabled a named profile is refused even if configured", () => {
	const disabled: TinysubagentConfig = { enableProfiles: false, profiles: { pro: { model: "m" } } };
...
test("availableProfileNames lists current first, then configured names sorted", () => {
	...
	assert.deepEqual(availableProfileNames({ enableProfiles: false, profiles: {} }), ["current"]);
});
...
test("the profile description degrades honestly when nothing is configured", () => {
	const text = profileParamDescription({ enableProfiles: true, profiles: {} });
